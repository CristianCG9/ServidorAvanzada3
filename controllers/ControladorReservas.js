...
export class ControladorReservas {
    ...
    async registrarReserva(peticion, respuesta) {
        ...
        let fechaFinal = new Date(datosReserva.fechaFinalReserva)
        let diferenciaEnMs = fechaFinal - fechaInicio
        ...
        try {
            ...
            let precioPorNoche = habitacion.precioNoche
            let precioTotal = precioPorNoche * diferenciaEnDias
            datosReserva.costo = precioTotal
            console.log(datosReserva)
            if (!habitacion) {
                respuesta.status(400).json({
                    "mensaje": `La habitación ${datosReserva.idHabitacion}, no existe`
                })
